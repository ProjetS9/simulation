	...
	public void operation(){
		...
		if(DS == 480){
			...
			for(int i=0; i<Variables.NC; i++){
				ClientCourriel c = new ClientCourriel(i);
				c.setDateArrC(480);
				Variables.listClientC.add(c);
			}
		...
		if(DS>=480 && DS<540){
			...
			Evenement arrC = new ArrCourriel(DS + loi.exponentielle(0.5));
			ToDoQueue.toDoQueue.add(arrC);
		}
		if(DS>=540 && DS<=720){
			//DS = DS + loi.exponentielle(1);
			Evenement arrC = new ArrCourriel(DS + loi.exponentielle(5));
			ToDoQueue.toDoQueue.add(arrC);
		}
	}
}
